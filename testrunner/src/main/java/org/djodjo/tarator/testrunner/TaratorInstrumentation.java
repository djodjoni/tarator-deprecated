...
return TaratorInstrumentation.super.startActivitySync(intent);
      }
    });
  ...
  private Instrumentation.ActivityResult getResultForBlockedIntent(Intent intent, int requestCode) {
    Log.i(LOG_TAG, "Intento: blocking external intent: " + intent);
    if (requestCode >= 0) {
      // Caller is expecting result.
      Instrumentation.ActivityResult result = intentSpy.getActivityResultForIntent(intent);
